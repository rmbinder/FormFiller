...
try {
    ...
    for ($conf = 0; $conf < $num_configs; $conf++)
    {
        ...
        $formConfigurations->addSelectBox('font'.$conf, $gL10n->get('PLG_FORMFILLER_FONT'), validFonts(), array('defaultValue' => $pPreferences->config['Formular']['font'][$conf], 'showContextDependentFirstEntry' => false));
        ...
        if ($num_configs > 1) 
        {
            $configuration['urlConfigDelete'] = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/formfiller/system/configurations.php', array('delete' => $conf + 1));
        }
        if (!empty('desc'.$conf)) 
        {
            $configuration['urlConfigCopy'] = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/formfiller/system/configurations.php', array('copy' => $conf + 1));
        }
    ...
    $page->assignSmartyVariable('urlConfigNew', SecurityUtils::encodeUrl(ADMIDIO_URL .FOLDER_PLUGINS . '/formfiller/system/configurations.php', array('add' => 1)));
    $page->assignSmartyVariable('urlPopupText',
        SecurityUtils::encodeUrl(
            ADMIDIO_URL . FOLDER_PLUGINS . '/formfiller/system/configurations_popup.php',
