...
/**
 ***********************************************************************************************
 * Creates and fills the PDF file for the Admidio plugin FormFiller
 *
...
try {
    ...
    $formID = admFuncVariableIsValid($_POST, 'form_id', 'numeric', array(
        'defaultValue' => 0
    ));
    $postListId = admFuncVariableIsValid($_POST, 'lst_id', 'numeric', array(
        'defaultValue' => 0
    ));
    $postPdfID = admFuncVariableIsValid($_POST, 'pdf_id', 'numeric', array(
        'defaultValue' => 0
    ));
    $postShowFormerMembers = admFuncVariableIsValid($_POST, 'show_former_members', 'bool', array(
        'defaultValue' => false
    ));
    ...
    if (isset($_POST['user_id']) && sizeof(array_filter($_POST['user_id'])) > 0) {
        ...
        if (sizeof($role_uuids_exclusion) > 0) {
            ...
            $sql = $list->getSQL(array(
                'showRolesMembers' => $role_uuids_exclusion,
                'showUserUUID' => true,
    ...
    } elseif (isset($_GET['kmf-RECEIVER'])) { // 'kmf-RECEIVER' ist als 'Systemfeld' deklariert, kann nicht geloescht werden und eignet sich deshalb hervorragend zur Ueberpruefung
        ...
        $formID = admFuncVariableIsValid($_GET, 'form_id', 'numeric', array(
            'defaultValue' => 0
        ));
        ...
        foreach ($etiketten as $data) {
            if (! (is_numeric($data))) {
                $etiketten = array(
                    ...
                    1,
                    0
                ); // das sind die Werte zum Drucken für eine Seite
                break;
            }
        }
    } else {
    ...
    foreach ($userScanArray as $userId) {
        ...
        if (! empty($pPreferences->config['Formular']['relation'][$formID]) && $user->getValue('GENDER', 'text') === $gL10n->get('SYS_MALE')) {
            ...
            $relationStatement = $gDb->queryPrepared($sql, array(
                $userId,
                $pPreferences->config['Formular']['relation'][$formID]
            ));
                ...
                if (! empty($formdata)) {
                    ...
                    switch ($fieldtype) {
                        ...
                        case 'm': // memberships
                            ...
                            foreach ($roleMemberships as $roleId) {
                                ...
                                if (array_key_exists('MS', $fontData) && strpos($fontData['MS'], 'B') !== false && strpos($fontData['MS'], 'E') !== false) {
                                    $sortArray[$pointer]['text'] .= ' (' . $gL10n->get('SYS_FROM_TO', array(
                                        $membership->getValue('mem_begin', $gSettingsManager->getString('system_date')),
                                        $membership->getValue('mem_end', $gSettingsManager->getString('system_date'))
                                    )) . ')';
                                } elseif (array_key_exists('MS', $fontData) && $fontData['MS'] === 'B') {
                                    $sortArray[$pointer]['text'] .= ' (' . $gL10n->get('SYS_FROM', array(
                                        $membership->getValue('mem_begin', $gSettingsManager->getString('system_date'))
                                    )) . ')';
                        ...
                        case 'f': // former memberships
                            ...
                            $formerRolesStatement = $gDb->queryPrepared($sql, array(
                                $userId,
                                DATE_NOW,
                                $gCurrentOrgId
                            ));
    ...
    // Info zu $pdf->Output()
    // bei Verwendung der Presenter funktioniert der Output bei Dest 'I' (default) und 'D' nicht mehr ('Some data has already been output, can't send PDF file')
    // Dest 'F' ist möglich, da hier die Funktion checkoutput() nicht aufgerufen wird
